...
class TestCongUREBase(unittest.TestCase):
    ...
    def assertSlowStart(self, state):
        ...
        """
        https://tools.ietf.org/html/draft-ietf-quic-recovery-34#section-7.3.1
    ...
    def assertCongestionAvoidance(self, state):
        ...
        # trying to be in line with `unittest`
        """
        https://tools.ietf.org/html/draft-ietf-quic-recovery-34#section-7.3.3
...
class TestCongUREQUICDefaultInitTests(TestCongUREBase):
    ...
    https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B
    ...
    def test_init(self):
        ...
        check if https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-A.4
        upholds:
...
class TestCongUREQUICDefault(TestCongUREBase):
    ...
    def test_on_packet_sent(self):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.4
        """
        state = self.cong_state()
    ...
    def test_on_packet_ack_no_rtt_sample(self, state=None):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-A.7
        """
        if state is None:
            state = self.cong_state()
        # From A.7, UpdateRtt
    ...
    def test_on_packet_ack_not_limited_slow_start(self):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.5
        """
        state = self.cong_state()
    ...
    def assertOnNewCongestionEvent(self, state, init_cwnd, init_ssthresh,
        ...
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.6
        """
        if persistent_congestion:
            # persistent congestion triggers return to slow start; see
            # https://tools.ietf.org/html/draft-ietf-quic-recovery-34#section-7.3
            self.assertNotInRecovery(state)
            # See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.8
    ...
    def test_on_ecn_information_not_recovery(self):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.7
        """
        state = self.cong_state()
        init_cwnd = state['cwnd']
    ...
    def test_on_packets_lost_not_recovery_pc(self):
        ...
        # See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#section-7.6.1
    ...
    def test_removing_discarded_packets_from_bytes_in_flight(self):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#appendix-B.9
        """
        state = self.cong_state()
        init_in_flight_size = state['in_flight_size']
    ...
    def test_pacing(self):
        """
        See https://tools.ietf.org/html/draft-ietf-quic-recovery-34#section-7.7
        """
        state = self.cong_state()
